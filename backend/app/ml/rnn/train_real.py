# File: backend/app/ml/rnn/train_real.py
# Vanilla RNN — baseline comparison model
# Architecture: Embedding → SpatialDropout → SimpleRNN × 2 → Attention → Dense → Output
# Run: python -m app.ml.rnn.train_real
...
from tensorflow.keras.layers import (
    Input, Embedding, SimpleRNN, Dense, Dropout, BatchNormalization,
...
print("\n" + "="*60)
print("  Vanilla RNN (Baseline) — Real Data Training")
print("="*60)
...
# SimpleRNN — vanishing gradient baseline
x   = SimpleRNN(RNN_UNITS, return_sequences=True,
                kernel_regularizer=l2(1e-5))(x)
x   = BatchNormalization()(x)
x   = Dropout(0.2)(x)
x   = SimpleRNN(RNN_UNITS // 2, return_sequences=True,
                kernel_regularizer=l2(1e-5))(x)
x   = BatchNormalization()(x)

attn     = BahdanauAttention(RNN_UNITS // 2)(x)
...
model = Model(inputs=inp, outputs=out, name='RNN_Baseline')
model.compile(
...
with open(REPORT_PATH, 'w') as f:
    f.write(f"Vanilla RNN (Baseline)\nVal Accuracy: {val_acc*100:.2f}%\n\n{report}")

fig, axes = plt.subplots(1, 2, figsize=(14, 5))
fig.suptitle('Vanilla RNN — Training Curves', fontsize=14, fontweight='bold')
axes[0].plot(history.history['accuracy'], label='Train', color='#6b7280', linewidth=2)
axes[0].plot(history.history['val_accuracy'], label='Val', color='#9ca3af', linewidth=2, linestyle='--')
...
plt.title('Vanilla RNN — Confusion Matrix', fontweight='bold')
plt.ylabel('True'); plt.xlabel('Predicted')
plt.tight_layout(); plt.savefig(CM_PATH, dpi=150); plt.close()

print(f"\n✅ RNN Baseline complete! Accuracy: {val_acc*100:.1f}%")

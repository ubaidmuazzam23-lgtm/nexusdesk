# File: backend/app/ml/lstm/train_real.py
# LSTM with Self-Attention — Real IT Ticket Complexity Classifier
# Architecture: Embedding → SpatialDropout → LSTM × 2 → Self-Attention → Dense → Output
# Run: python -m app.ml.lstm.train_real
...
print("\n" + "="*60)
print("  LSTM + Attention — Real Data Training")
print("="*60)

import pickle as pkl
df = __import__('pandas').read_csv(DATA_PATH).dropna(subset=['text','complexity'])
...
X     = __import__('tensorflow').keras.preprocessing.sequence.pad_sequences(
    tok.texts_to_sequences(df['text'].tolist()), maxlen=MAX_LEN, padding='post', truncating='post')
...
x   = LSTM(LSTM_UNITS, return_sequences=True, dropout=0.2,
           kernel_regularizer=l2(1e-5))(x)
x   = BatchNormalization()(x)
x   = LSTM(LSTM_UNITS // 2, return_sequences=True, dropout=0.2,
           kernel_regularizer=l2(1e-5))(x)
x   = BatchNormalization()(x)

attn     = BahdanauAttention(LSTM_UNITS // 2)(x)
...
axes[0].plot(history.history['val_accuracy'], label='Val', color='#60a5fa', linewidth=2, linestyle='--')
...
plt.title('LSTM + Attention — Confusion Matrix', fontweight='bold')
plt.ylabel('True'); plt.xlabel('Predicted')
plt.tight_layout(); plt.savefig(CM_PATH, dpi=150); plt.close()

print(f"\n✅ LSTM + Attention complete! Accuracy: {val_acc*100:.1f}%")

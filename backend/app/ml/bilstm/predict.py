...
def _load():
    ...
    if not os.path.exists(MODEL_PATH) or not os.path.exists(TOKENIZER_PATH):
        raise FileNotFoundError("BiLSTM model not trained yet")
...
def predict_complexity(text: str) -> dict:
    ...
    result = _model.predict(padded, verbose=0)
    probs  = result[0] if isinstance(result, (list, tuple)) else result[0]
